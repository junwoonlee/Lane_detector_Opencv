...
video_file = "C:/Users/JUN/Desktop/DR PR/2021.11 CV_car_driving_Project/video_driving.mp4"
cap = cv2.VideoCapture(video_file) 
width = cap.get(cv2.CAP_PROP_FRAME_WIDTH) 
height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT) 
fps = cap.get(cv2.CAP_PROP_FPS) 
print('Frame_width: %d, Frame_height: %d, Frame_fps: %d' %(width, height, fps))

vertices = np.array([[(500, height - 130),(900, height - 400), (width-900, height - 400), (width-300, height - 130)]], dtype=np.int32)
# vertices = np.array([[(400, height - 100),(800, height - 300), (width-800, height - 350), (width-300, height - 120)]], dtype=np.int32)
L_line = []
R_line = []
...
while(cap.isOpened()):
    ...
    tr_img = draw_lines(ROI_GBimage, Regressed_lines)
    # ROI_GBimage, image_raw
    cv2.imshow('frame', tr_img)
